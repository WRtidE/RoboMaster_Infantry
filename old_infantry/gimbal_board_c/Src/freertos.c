...
// gimbal_up 5614   gimbal_middle = 7328  gimbal_down = 7925
void gimbal_pitch_task(void const * argument)
{
  ...
  for(;;)
  {
    if(can_flag ==1)
    {
      ...
      {
        if (motor_info[3].rotor_speed > 10 || motor_info[3].rotor_speed < -10) //ï¿½ï¿½ï¿½ï¿½(?)
        {
          target_speed[3] = 0;
      ...
      {
        if(rc_ctrl.rc.ch[1]>=364&&rc_ctrl.rc.ch[1]<=824 &&motor_info[3].rotor_angle < 7800) //Ò£¿ØÆ÷ÍùÏÂ²¦
        {
          target_speed[3] = -((rc_ctrl.rc.ch[1] - 1024) / 660 * 16);
          motor_info[3].set_voltage = pid_calc(&motor_pid[3], target_speed[3], motor_info[3].rotor_speed);
          shoot_flag = 2;
        }
        if(rc_ctrl.rc.ch[1]>=1224&&rc_ctrl.rc.ch[1]<=1684&& motor_info[3].rotor_angle>5800) //Ò£¿ØÆ÷ÍùÉÏ²¦
        {
          target_speed[3] = -((rc_ctrl.rc.ch[1] - 1024) / 660 * 16);
          motor_info[3].set_voltage = pid_calc(&motor_pid[3], target_speed[3], motor_info[3].rotor_speed);
          shoot_flag = 2;
