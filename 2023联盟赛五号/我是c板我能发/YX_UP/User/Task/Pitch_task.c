    ...
    if(rc_ctrl.rc.s[0] == 1)
    {
	  ...
	  if (!rc_ctrl.mouse.y)
	  {
			...
			else
		  {
				...
				if(rc_ctrl.rc.ch[1]<=660 && (motor_info[4].rotor_angle > 7000 || motor_info[4].rotor_angle < 1600 )) //遥控器往上拨
				{
					target_speed[4] = -((rc_ctrl.rc.ch[1]) / 660.0*16);
					motor_info[4].set_voltage = pid_calc(&motor_pid[4], target_speed[4], motor_info[4].rotor_speed);
					shoot_flag = 2;
	  ...
	  else
	  {
		  ...
		  else
		  {
				...
				if(rc_ctrl.mouse.y>5 && rc_ctrl.mouse.y <= 16384 && (motor_info[4].rotor_angle > 7000 || motor_info[4].rotor_angle < 1600 )) //遥控器往上拨
				{
					target_speed[4] =  rc_ctrl.mouse.y / 16384.00 *3000;
					motor_info[4].set_voltage = pid_calc(&motor_pid[4], target_speed[4], motor_info[4].rotor_speed);
					shoot_flag = 2;
